...
public class CurlTest extends TestSetUp{
  
  /**
   * test downloading the file from the example URL in 2B handout
   */
  @Test
  public void test1execute(){
    Curl curl = new Curl();
    curl.setParameters("http://www.cs.cmu.edu/~spok/grimmtmp/073.txt");
    curl.execute();
    assertEquals(DirectoryList.getTxtFile("073txt").getContent(), 
  ...
  /**
   * test giving an invalid URL
   */
  @Test
  public void test2execute(){
    Curl curl = new Curl();
    curl.setParameters("jibsdsfnnsfjdn");
    curl.execute();
    assertEquals(outContent.toString(), "Can't get the file\r\n");
  }
}
